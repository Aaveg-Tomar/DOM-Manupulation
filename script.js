const button = document.getElementById("addAlbumButton");   // get the buttun
const albums = document.getElementsByClassName("albums")[0];   // get the albums



// when the button is click we call this fuction

button.addEventListener('click',function() {
    
    const image = document.getElementById("imageURL").value;

    console.log(image);
    const description = document.getElementById("description").value;   
    const time = document.getElementById("time").value;


    // create new function for the div which contain album discription and time

   const album = createNewAlbum(image, description , time);  // here we call a function
   
   albums.appendChild(album);

});

// Here we create a function

function createNewAlbum(imageURL , description , time)
{
    const albumDiv = document.createElement('div');     // Here we create a album div
    albumDiv.className = "album";                    // Here we give div the class name
    
    

    const thumbnailDiv = document.createElement('div');  // here we created a thumbanail div
    thumbnailDiv.className = "thumbnail";

    const image = document.createElement('img');           // here we created a image tag
    image.setAttribute('src',imageURL);                  // for adding an image we use setAttribute and "scr is for link" and "imageURL is a value input"
    thumbnailDiv.appendChild(image);                     // here we put image(child) inside the thubnailDiv(parent) 

    


    const details = document.createElement('div');      // here we created a detailsDiv div
    details.className = "details";                      // give the class name to the details
    const para = document.createElement('p');
    para.textContent = description;

     
    const detailsFooter = document.createElement('div');    // created details footer
    detailsFooter.className = "details-footer";             // give class name to the details-footer

    const buttondiv = document.createElement('div');        // created button div 
    const button1 = document.createElement('button');       // created button1
    button1.textContent = "View";                           // add text
    const button2 = document.createElement('button');       // created button2
    button2.textContent = "Edit";                           // add text

    buttondiv.appendChild(button1);                          // apend button1 into buttondiv
    buttondiv.appendChild(button2);                          // apend button2 into buttondiv          

    const timeElement = document.createElement('span');
    timeElement.textContent= time + " min";

    detailsFooter.appendChild(buttondiv);
    detailsFooter.appendChild(timeElement);

    details.appendChild(para);
    details.appendChild(detailsFooter);
        
    albumDiv.appendChild(thumbnailDiv);
    albumDiv.appendChild(details);   

    // this is for reference for making above script

    // <div class="album">
    //             <div class="thumbnail" >
    //                 <img class="round" src="data:image/jpeg;base64," />  
    //             </div>
        
    //             <div class="details">
    //                 <p>
    //                     This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.
    //                 </p>
        
    //                 <div class="details-footer" >
    //                     <div>
    //                         <button>View</button>
    //                         <button>Edit</button>
    //                     </div>
    //                     <span>9 min </span>
    //                 </div>
    //             </div>
    //         </div>
    

    return albumDiv;
}   






